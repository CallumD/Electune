...
describe SpotifySongSearch, "the spotify search helper" do
  ...
  describe "lookup by artist" do
    before(:each) do
      SpotifySongSearch.stub(:get_url_response).and_return({'artist' => { 'albums' => [ 'album' => {'name' => 'album_name', 'released' => "2012-12-01 09:04:35", 'href' => 'album.link'}]}})
    end
  ...
  describe "lookup by album" do
    before(:each) do
      SpotifySongSearch.stub(:get_url_response).and_return({'album' => { 'name' => 'album_name', 'released' => "2012-12-01 09:04:35", 'href' => 'album.link', 'tracks' => [{'name'=> 'name', 'length' => 3600, 'href' => 'test.link', 'artists' => [{'name'=> 'name', 'href' => 'test.link'}]}]}})
    end
