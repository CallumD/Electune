...
describe PlaylistItem, "#voting" do
  ...
  it "should only allow upvoting when it has at least one vote" do
    playlist_item.veto playlist_item.user._id
    playlist_item.upvote playlist_item.user._id
  ...
  it "should decreate the votes when vetoed" do
   playlist_item.upvote FactoryGirl.create(:user)._id
   playlist_item.veto user._id
   playlist_item.votes.should eq(1)
  end
  ...
  it "should persist an upvote" do
    ...
    playlist_item.upvote user._id
    from_database = PlaylistItem.find playlist_item._id
  ...
  it "should return false if not vetoed by user" do
     ...
     playlist_item.already_vetoed_by_user?(non_vetoed_user._id).should eq false
  end

  it "should return true if not upvoted by user" do
     playlist_item = FactoryGirl.create(:playlist_item)
     playlist_item.veto playlist_item.user._id
     playlist_item.already_vetoed_by_user?(playlist_item.user._id).should eq true
  end
end
